const Importer = {
    ...
    openMappingModal() {
        ...
        // הוספת בחירת קבוצה עם פרי-סלקט חכם
        if (this.currentType === 'donors') {
            ...
            if (window.Donors && Donors.viewTab !== 'all' && Donors.viewTab !== 'unassigned') {
                ...
                if(groupSelect && groupSelect.value) {
                    preselectGroupVal = `${Donors.viewTab}|${groupSelect.value}`;
                }
            }
            ...
            html += `<div class="bg-emerald-50 text-emerald-900 p-3 rounded mb-4 text-sm font-bold border border-emerald-200">
                <label class="block mb-1"><i class="fas fa-users"></i> שיוך תורמים לקבוצה (אופציונלי):</label>
        ...
        if (isDonationImport) {
            ...
            years.forEach(hYear => {
                ...
                html += `<div class="grid grid-cols-2 gap-4 items-center mb-1"><div class="text-sm">תרומות ${hYear} (${numYear})</div><select id="map-finance-${numYear}" class="border rounded p-1 w-full text-sm bg-gray-50 history-selector" data-year="${numYear}" data-hyear="${hYear}">${options}</select></div>`;
            });
            html += `</div></div>`;
        }
        html += `</div>`; 
    ...
    executeImport() {
        ...
        document.querySelectorAll('.history-selector').forEach(el => {
            if (el.value) historyMapping[el.dataset.year] = { col: el.value, hYear: el.dataset.hyear };
        });
        ...
        db.ref(dbPath).once('value', snapshot => {
            ...
            this.fileData.forEach(row => {
                ...
                if (this.currentType !== 'students_donations') {
                    if (!entityId) {
                        ...
                        if (this.currentType === 'donors' && selectedGroup) {
                            newDonorIdsForGroup.push(entityId);
                        }
                    }
                } else {
                    if (!entityId) return; 
                ...
                if (entityId) {
                    Object.entries(historyMapping).forEach(([numYear, mapData]) => {
                        let val = row[mapData.col];
                        if (!val) return;
                        
                        const txId = 'imp' + Date.now() + Math.random().toString(36).substr(2, 5);
                        const yearInt = parseInt(numYear); 
                        const gregorianYear = yearInt - 3760;
                        
                        const tx = {
                            id: txId,
                            date: new Date(gregorianYear, 2, 14).getTime(),
                            type: 'income',
                            category: 'יבוא אקסל',
                            desc: 'היסטוריה/יבוא',
                            amount: val,
                            isPurim: true,
                            importedYear: mapData.hYear
                        };
                        
                        if (this.currentType === 'donors') tx.donorId = entityId;
                        else tx.studentId = entityId;
                        
                        financeUpdates[`years/${mapData.hYear}/finance/${txId}`] = tx;
                        countFinance++;
                    });
                }
            });
            ...
            if (newDonorIdsForGroup.length > 0 && selectedGroup) {
                const [day, gid] = selectedGroup.split('|');
            ...
            Promise.all(promises).then(() => {
                ...
                if (newDonorIdsForGroup.length > 0) msg += ` ${newDonorIdsForGroup.length} תורמים שובצו לקבוצה.`;
