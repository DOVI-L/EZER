const Reports = {
    ...
    openEditor(mode = 'visual', payload = '') {
        ...
        this.editorState.colsVisible = [];
        this.editorState.headersOrder = [];

        if (mode === 'custom') {
            this.editorState.customHtml = payload;
            this.editorState.title = 'תצוגה מקדימה להדפסה';
        } else if (mode === 'route') {
            this.editorState.routeData = payload.data;
            this.editorState.title = payload.title || 'דף מסלול';
            this.editorState.subTitle = payload.subTitle || '';
            this.editorState.headers = payload.headers || [];
            this.editorState.colsVisible = payload.colsVisible || [];
            this.editorState.headersOrder = payload.headersOrder || [];
        } else {
            this.editorState.title = 'דוח תצוגה משוקלל';
    ...
    async renderVisualMode(container) {
        ...
        try {
            ...
            const sortType = this.editorState.sortBy || 'amount_desc';
            list.sort((a,b) => {
                 if (sortType === 'amount_desc') return (b.totalDisplay || 0) - (a.totalDisplay || 0);
                 if (sortType === 'goal_desc') return (b.goal || 0) - (a.goal || 0);
            ...
            chunks.forEach((chunk, pageIdx) => {
                let pageHtml = `
                <div class="report-page ${pageClass} editor-print-mode" style="transform: scale(${this.editorState.zoom || 0.6}); padding: ${this.editorState.marginPadding || 20}px;">
                    <div class="print-layer-bg bg-contain bg-center bg-no-repeat" style="${bgStyle} opacity: ${this.editorState.bgOpacity || 0.2};"></div>
                    <div class="print-layer-content h-full flex flex-col relative">
                        <div class="print-header mb-4 relative">
                            <div class="draggable-item" style="position: absolute; right: 0; top: 0; width: 60px;">
                                 <img src="1.JPG" alt="לוגו" class="w-full h-full object-contain pointer-events-none">
